...
router.get('/', authenticate, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    return res.json({ success: true, bookings: rows, total, page: parseInt(page), limit: parseInt(limit) })
  } catch (err) {
    return res.status(500).json({ success: false, message: 'Server error.' })
  }
})
...
router.put('/:id', authenticate, requireAdmin, async (req, res) => {
  ...
  try {
    await pool.query(
      'UPDATE bookings SET status = COALESCE(?, status), admin_notes = COALESCE(?, admin_notes) WHERE id = ?',
